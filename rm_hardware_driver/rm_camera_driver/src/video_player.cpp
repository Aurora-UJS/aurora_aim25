...
public:
  ...
  : Node("camera_driver", options), frame_cnt_(0) {
    ...
    AURORA_INFO("camera_driver", "Starting VideoPlayerNode!");
    // Get parameters
    ...
    if (!std::filesystem::exists(video_file)) {
      AURORA_ERROR("camera_driver", "Video file {} does not exist!", video_file.string());
      rclcpp::shutdown();
      return;
    }
    cap_.open(video_path);
    if (!cap_.isOpened()) {
      AURORA_ERROR("camera_driver", "Video file open failed!");
      rclcpp::shutdown();
      return;
    }
    ...
    if (camera_info_manager_->validateURL(camera_info_url)) {
      ...
      AURORA_WARN("camera_driver", "Invalid camera info URL: {}", camera_info_url);
    }
    ...
    timer_ = this->create_wall_timer(std::chrono::milliseconds(1), [this]() {
      cap_ >> frame_;
      if (frame_.empty()) {
        AURORA_INFO("camera_driver", "Video file ends!");
        if (!is_loop_) {
          rclcpp::shutdown();
          return;
        } else {
      ...
      if (frame_cnt_ < start_frame_) {
        AURORA_INFO("camera_driver", "Skipping frame {}", frame_cnt_);
        return;
      }
...
RCLCPP_COMPONENTS_REGISTER_NODE(aurora::camera_driver::VideoPlayerNode)
