...
namespace aurora::camera_driver {
DahengCameraNode::DahengCameraNode(const rclcpp::NodeOptions &options)
: Node("camera_driver", options) {
  AURORA_REGISTER_LOGGER("camera_driver", "~/aurora-log", INFO);
  AURORA_INFO("camera_driver", "Starting DahengCameraNode!");
  ...
  if (pixel_format_ == "mono8") {
    ...
    AURORA_ERROR("camera_driver", "Unsupported pixel format: {}", pixel_format_);
  }
  ...
  if (camera_info_manager_->validateURL(camera_info_url_)) {
    ...
    AURORA_WARN("camera_driver", "Invalid camera info URL: {}", camera_info_url_);
  }
  ...
  if (enable_recorder) {
    ...
    std::filesystem::path video_path = fs::path(home) / "aurora2026-log/video/" /
                                       std::string(std::to_string(std::time(nullptr)) + ".avi");

    recorder_ = std::make_unique<Recorder>(
      video_path, frame_rate_, cv::Size(resolution_width_, resolution_height_));
    recorder_->start();
    AURORA_INFO("camera_driver", "Recorder started! Video file: {}", video_path.string());
  }

  AURORA_INFO("camera_driver", "DahengCameraNode has been initialized!");
}
...
DahengCameraNode::~DahengCameraNode() {
  ...
  AURORA_INFO("camera_driver", "DahengCameraNode has been destroyed!");
}
...
void DahengCameraNode::timerCallback() {
  ...
  while (!is_open_ && rclcpp::ok()) {
    ...
    if (!is_open_success) {
      AURORA_ERROR("camera_driver", "open() failed");
      close();
      return;
    }
  }
  ...
  if (dt > 5.0) {
    AURORA_WARN("camera_driver", "Camera is not alive! lost frame for {:.2f} seconds", dt);
    close();
  }
}

void DahengCameraNode::close() {
  AURORA_INFO("camera_driver", "Closing Daheng Galaxy Camera Device!");
...
bool DahengCameraNode::open() {
  AURORA_INFO("camera_driver", "Opening Daheng Galaxy Camera Device!");
  if (is_open_) {
    AURORA_WARN("camera_driver", "Daheng Galaxy Camera Device is already opened!");
    close();
  }
  ...
  if (gx_status_ != GX_STATUS_SUCCESS) {
    AURORA_ERROR("camera_driver", "Can't init lib");
    return false;
  }
  ...
  if ((gx_status_ != GX_STATUS_SUCCESS) || (device_num <= 0)) {
    AURORA_WARN("camera_driver", "Can't find camera");
    return false;
  }
  AURORA_INFO("camera_driver", "Found {} devices", device_num);
  //打开设备
  gx_status_ = GXOpenDevice(&openParam, &dev_handle_);
  if (gx_status_ != GX_STATUS_SUCCESS) {
    AURORA_ERROR("camera_driver", "Can't open device");
    return false;
  }
  is_open_ = true;
  ...
  if (gx_status_ != GX_STATUS_SUCCESS) {
    AURORA_ERROR("camera_driver", "Register capture callback function failed!");
    return false;
  }
  ...
  if (gx_status_ != GX_STATUS_SUCCESS) {
    AURORA_ERROR("camera_driver", "Send start capture command failed!");
    return false;
  }
  AURORA_INFO("camera_driver", "Daheng Galaxy Camera Device Open Success!");
  return true;
}
...
rcl_interfaces::msg::SetParametersResult DahengCameraNode::onSetParameters(
  ...
  for (const auto &param : parameters) {
    if (param.get_name() == "exposure_time") {
      ...
      AURORA_INFO("camera_driver", "Set exposure_time: {}", exposure_time_);
    } else if (param.get_name() == "gain") {
      gain_ = param.as_double();
      GXSetFloat(dev_handle_, GX_FLOAT_GAIN, gain_);
      AURORA_INFO("camera_driver", "Set gain: {}", gain_);
    }
  }
  return result;
}
...
RCLCPP_COMPONENTS_REGISTER_NODE(aurora::camera_driver::DahengCameraNode)
