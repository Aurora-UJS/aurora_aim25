...
using namespace aurora::serial_driver;
// FIFO传输设备，用于测试。
...
public:
  ...
  int read(void *buffer, size_t len) override { return ::read(fifo_rd_fd_, buffer, len); }
  int write(const void *buffer, size_t len) override { return ::write(fifo_wr_fd_, buffer, len); }
...
public:
  ...
  TransporterInterface::SharedPtr get_transporter1() { return transporter1_; }
  TransporterInterface::SharedPtr get_transporter2() { return transporter2_; }
