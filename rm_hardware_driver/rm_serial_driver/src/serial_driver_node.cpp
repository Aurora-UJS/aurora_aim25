...
void SerialDriverNode::init() {
  AURORA_INFO("serial_driver", "Initializing SerialDriverNode!");
  // Init
  ...
  if (protocol_ == nullptr) {
    AURORA_FATAL("serial_driver", "Failed to create protocol with type: {}", protocol_type);
    rclcpp::shutdown();
    return;
  }
  AURORA_INFO(
  ...
  for (auto sub : subscriptions_) {
    AURORA_INFO("serial_driver", "Subscribe to topic: {}", sub->get_topic_name());
  }
  // Publisher
  ...
  for (auto client : protocol_->getClients(this->shared_from_this())) {
    ...
    AURORA_INFO("serial_driver", "Create client for service: {}", name);
  }

  // Heartbeat
  heartbeat_ = HeartBeatPublisher::create(this);

  AURORA_INFO("serial_driver", "SerialDriverNode has been initialized!");
}
...
void SerialDriverNode::listenLoop() {
  ...
  while (rclcpp::ok()) {
    if (protocol_->receive(receive_data)) {
      ...
      AURORA_WARN("serial_driver", "Failed to reveive packet! error message :{}", error_message);
...
void SerialDriverNode::setMode(SetModeClient &client, const uint8_t mode) {
  ...
  while (!client.ptr->wait_for_service(1s)) {
    ...
    AURORA_INFO("serial_driver", "Service {} not available, waiting again...", service_name);
  }
