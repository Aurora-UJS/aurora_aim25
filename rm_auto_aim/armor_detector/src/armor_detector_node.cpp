...
namespace aurora::auto_aim {
ArmorDetectorNode::ArmorDetectorNode(const rclcpp::NodeOptions &options)
: Node("armor_detector", options) {
  AURORA_REGISTER_LOGGER("armor_detector", "~/aurora2026-log", INFO);
  AURORA_INFO("armor_detector", "Starting ArmorDetectorNode!");
  // Detector
...
void ArmorDetectorNode::imageCallback(const sensor_msgs::msg::Image::ConstSharedPtr img_msg) {
  ...
  try {
    ...
    auto odom_to_gimbal = tf2_buffer_->lookupTransform(
      odom_frame_, img_msg->header.frame_id, target_time, rclcpp::Duration::from_seconds(0.01));
    ...
    AURORA_ERROR("armor_detector", "Something Wrong when lookUpTransform");
    return;
  }
  ...
  if (armor_pose_estimator_ != nullptr) {
    ...
    // std::string path =
    //   fmt::format("/home/xlqmu/aurora2026-log/images/{}/{}.jpg",
    //   armor_msg.number, now().seconds());
    // cv::imwrite(path, armor.number_img);
  } else {
    AURORA_WARN("armor_detector", "PnP Failed!");
  }
...
std::unique_ptr<Detector> ArmorDetectorNode::initDetector() {
  ...
  AURORA_ASSERT_MSG(fs::exists(model_path) && fs::exists(label_path),
...
void ArmorDetectorNode::setModeCallback(
  ...
  if (mode_name == "UNKNOWN") {
    AURORA_ERROR("armor_detector", "Invalid mode: {}", request->mode);
    return;
  }
  ...
  AURORA_WARN("armor_detector", "Set mode to {}", mode_name);
}
...
RCLCPP_COMPONENTS_REGISTER_NODE(aurora::auto_aim::ArmorDetectorNode)
