...
: Node("armor_solver", options), solver_(nullptr) {
  // Register logger
  AURORA_REGISTER_LOGGER("armor_solver", "~/aurora2026-log", INFO);
  AURORA_INFO("armor_solver", "Starting ArmorSolverNode!");
...
void ArmorSolverNode::timerCallback() {
  ...
  if (armor_target_.tracking) {
    try {
      ...
      AURORA_ERROR("armor_solver", "Something went wrong in solver!");
      control_msg.yaw_diff = 0;
      control_msg.pitch_diff = 0;
      control_msg.distance = -1;
      control_msg.fire_advice = false;
...
void ArmorSolverNode::armorsCallback(const rm_interfaces::msg::Armors::SharedPtr armors_msg) {
  ...
  for (auto &armor : armors_msg->armors) {
    ...
    try {
      ...
      AURORA_ERROR("armor_solver", "Transform error: {}", ex.what());
      return;
    }
  }
...
void ArmorSolverNode::setModeCallback(
  ...
  if (mode_name == "UNKNOWN") {
    AURORA_ERROR("armor_solver", "Invalid mode: {}", request->mode);
    return;
  }
  ...
  AURORA_WARN("armor_solver", "Set Mode to {}", visionModeToString(mode));
}
...
RCLCPP_COMPONENTS_REGISTER_NODE(aurora::auto_aim::ArmorSolverNode)
