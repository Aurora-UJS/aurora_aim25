...
Solver::Solver(std::weak_ptr<rclcpp::Node> n) : node_(n) {
  ...
  if (!manual_compensator_->updateMapFlow(angle_offset)) {
    AURORA_WARN("armor_solver", "Manual compensator update failed!");
  }
...
rm_interfaces::msg::GimbalCmd Solver::solve(const rm_interfaces::msg::Target &target,
  ...
  try {
    ...
    AURORA_ERROR("armor_solver", "{}", e.what());
  }
  ...
  try {
    ...
    AURORA_ERROR("armor_solver", "{}", ex.what());
    throw ex;
  }
  ...
  if (gimbal_cmd.fire_advice) {
    AURORA_DEBUG("armor_solver", "You Need Fire!");
  }
  return gimbal_cmd;
}
