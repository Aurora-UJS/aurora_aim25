...
void Tracker::init(const Armors::SharedPtr &armors_msg) noexcept {
  ...
  AURORA_INFO("armor_solver", "Init EKF!");
...
void Tracker::update(const Armors::SharedPtr &armors_msg) noexcept {
  ...
  if (!armors_msg->armors.empty()) {
    ...
    if (min_position_diff < max_match_distance_ && yaw_diff < max_match_yaw_diff_) {
      ...
      AURORA_WARN("armor_solver", "No matched armor found!");
    }
  }
  ...
  if (tracker_state == DETECTING) {
    if (matched) {
      ...
      if (detect_count_ > tracking_thres) {
        ...
        AURORA_DEBUG("armor_solver", "Tracker state: TRACKING {}", tracked_id);
      }
    } else {
      detect_count_ = 0;
      tracker_state = LOST;
      AURORA_DEBUG("armor_solver", "Tracker state: LOST {}", tracked_id);
    }
  } else if (tracker_state == TRACKING) {
    if (!matched) {
      tracker_state = TEMP_LOST;
      lost_count_++;
      AURORA_DEBUG("armor_solver", "Tracker state: TEMP_LOST {}", tracked_id);
    }
...
void Tracker::handleArmorJump(const Armor &current_armor) noexcept {
  ...
  if (abs(yaw - last_yaw) > 0.4) {
    ...
    AURORA_DEBUG("armor_solver", "Armor Jump!");
  }
  ...
  if ((current_p - infer_p).norm() > max_match_distance_) {
    ...
    AURORA_WARN("armor_solver", "State wrong!");
  }
