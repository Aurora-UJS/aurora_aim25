...
namespace aurora::rune {
RuneDetectorNode::RuneDetectorNode(const rclcpp::NodeOptions &options)
: Node("rune_detector", options), is_rune_(false) {
  AURORA_REGISTER_LOGGER("rune_detector", "~/aurora2026-log", INFO);
  AURORA_INFO("rune_detector", "Starting RuneDetectorNode!");
...
std::unique_ptr<RuneDetector> RuneDetectorNode::initDetector() {
  ...
  AURORA_ASSERT(!model_path.empty());
  AURORA_INFO("rune_detector", "model : {}, device : {}", model_path, device_type);
  ...
  AURORA_ASSERT_MSG(fs::exists(resolved_path), resolved_path.string() + " Not Found");
...
void RuneDetectorNode::inferResultCallback(std::vector<RuneObject> &objs,
  ...
  if (!objs.empty()) {
    ...
    if (result_it != objs.end()) {
      // AURORA_DEBUG("rune_detector", "Detected!");
...
void RuneDetectorNode::setModeCallback(
  ...
  if (mode_name == "UNKNOWN") {
    AURORA_ERROR("rune_detector", "Invalid mode: {}", request->mode);
    return;
  }
  ...
  AURORA_WARN("rune_detector", "Set Rune Mode: {}", visionModeToString(mode));
}
...
RCLCPP_COMPONENTS_REGISTER_NODE(aurora::rune::RuneDetectorNode)
