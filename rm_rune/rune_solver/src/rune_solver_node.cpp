...
RuneSolverNode::RuneSolverNode(const rclcpp::NodeOptions &options) : Node("rune_solver", options) {
  AURORA_REGISTER_LOGGER("rune_solver", "~/aurora2026-log", INFO);
  AURORA_INFO("rune_solver", "Starting RuneSolverNode!");
...
void RuneSolverNode::timerCallback() {
  ...
  if (rune_solver_->tracker_state == RuneSolver::TRACKING) {
    ...
    try {
      ...
      AURORA_ERROR("rune_solver", "solveGimbalCmd error");
      control_msg.yaw_diff = 0;
      control_msg.pitch_diff = 0;
      control_msg.distance = -1;
      control_msg.pitch = 0;
...
void RuneSolverNode::setModeCallback(
  ...
  if (mode_name == "UNKNOWN") {
    AURORA_ERROR("rune_solver", "Invalid mode: {}", request->mode);
    return;
  }
  ...
  AURORA_WARN("rune_solver", "Set Rune Mode: {}", visionModeToString(mode));
}
...
RCLCPP_COMPONENTS_REGISTER_NODE(aurora::rune::RuneSolverNode)
